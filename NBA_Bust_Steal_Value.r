...
df_redraft2 <- Reduce(x = tables_test, f = rbind)
str(df_redraft2)           
...
df_BustSteal2 <- Reduce(x = tables_test2, f = rbind)
df_BustSteal2 <- na.omit(df_BustSteal2)
tail(df_BustSteal2, 30)

#Team Bust/Steal Value
team <- unique(df_BustSteal2$Tm)
tables_test3 = lapply(team ,function(tm){
...
df_teamVORPsum <- Reduce(x = tables_test3, f = rbind)
df_teamVORPsum
